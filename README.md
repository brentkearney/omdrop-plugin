...
- **Sending**: click a name in the list, or any dot on the radar, and choose a file. A dot works before its name is known; its row is then labelled by address. The chooser opens in the folder you last sent from, or in the download folder the first time and whenever that folder is gone. The row shows the send's progress.
